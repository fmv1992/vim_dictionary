...
MessageContent = namedtuple(
    "message_content", ["lookup_word", "language", "textwidth"]
)


LOGGING_FORMAT = logging.Formatter(
    fmt=(
        "|Thread: %{threadName:18s}|{levelname:8s}|{asctime:s}|{name:12s}"
        "| {message:s}"
    ),
    datefmt="%Y-%m-%d %H:%M:%S",
    style="{",
...
def parse_arguments():
    ...
    parser.add_argument(
        "--dictionary",
        help=(
            "Set the dictionary to be used. Implemented options are: "
            "{0}.".format(
                ",".join(["'wikitionary' (online)", "'webster' (offline)"])
            )
        ),
        default="wikitionary",
        required=True,
