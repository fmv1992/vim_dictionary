...
class WebsterDictionary(VimDictionary):
    ...
    DICT_ENTRY_REGEX = re.compile(
        r"^[A-Z ;-]+$", flags=(re.MULTILINE | re.VERBOSE)
    )
    ...
    def _lookup(self, entry, language):
        ...
        if language != "english":
            raise ValueError(
                ...
                " English only dictionary.".format(word=entry, lang=language)
            )
        entry = entry.upper()
        ...
        if next_different_entry:
            ...
            return [
                "'{0}' not in the dictionary.".format(entry),
            ]
            ...
            for i1, entry in enumerate(wikiresult, start=1):
                ...
                for i2, definition in enumerate(entry["definitions"], start=1):
                    ...
                    one_line = "{entry_number}. {content}".format(
                        entry_number=i2, content=text.capitalize()
                    )
